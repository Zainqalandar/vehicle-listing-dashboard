    ...
    return (
        <nav className={`p-0 sticky top-0 z-50 shadow-md ${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-800'} text-white`}>
            <div className="max-w-7xl mx-auto flex justify-between items-center px-8 py-4">
                <Link to="/" className="text-xl font-bold text-white no-underline transition-colors duration-300 hover:text-blue-400" onClick={() => setIsOpen(false)}>
                    Vehicle Dashboard
                </Link>
                <div className="flex items-center gap-4">
                    ...
                    <div className={`flex flex-col md:flex-row md:static absolute top-16 left-0 w-full md:w-auto transition-all duration-300 ${isOpen ? 'left-0' : '-left-full md:left-auto'} ${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-800'} md:bg-transparent`}>
                        <Link to="/" className="text-white no-underline px-4 py-2 mx-2 my-1 md:my-0 rounded transition-all duration-300 hover:bg-gray-700 hover:text-blue-400" onClick={() => setIsOpen(false)}>
                            Home
                        </Link>
                        <Link to="/user-form" className="text-white no-underline px-4 py-2 mx-2 my-1 md:my-0 rounded transition-all duration-300 hover:bg-gray-700 hover:text-blue-400" onClick={() => setIsOpen(false)}>
                            User Form
                        </Link>
                    </div>
                    <button
